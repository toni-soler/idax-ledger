...
test("Overview/Nodes/Ledgers/Transactions never index an undefined network",()=>{assert.doesNotMatch(source,/ns\.find\(x=>x\.enabled\)\|\|ns\[0\]/);assert.doesNotMatch(source,/const n=ns\.find\(x=>x\.enabled\);(?!.*enabledNetwork)/);assert.match(source,/const enabledNetwork=async/);assert.match(source,/NoEnabledNetwork/)});
test("no-enabled-network message is translated for every locale, not left English-only",async()=>{const es=JSON.parse(await readFile(new URL("../src/locales/es.json",import.meta.url),"utf8"));assert.ok(es.ledger.noEnabledNetwork);assert.ok(es.ledger.errors?.forbidden)});
